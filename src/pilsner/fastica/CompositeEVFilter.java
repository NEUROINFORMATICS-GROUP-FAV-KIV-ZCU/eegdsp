...
/**
 * The <code>CompositeEVFilter</code> can be used to build a chain of
 * eigenvalue filters.
 * 
 * @author Michael Lambertz
 */

public class CompositeEVFilter extends ArrayList<EigenValueFilter> 
	...
	public CompositeEVFilter(EigenValueFilter evf1, EigenValueFilter evf2) {
		super(2);
		set(0, evf1);
		set(1, evf2);
	}
	...
	public void passEigenValues(double[] eigenValues, double[][] eigenVectors) {
		if (size() == 0) {
			...
			for (int i = 0; i < size() - 1; ++i)
				get(i + 1).passEigenValues(get(i).getEigenValues(),
						get(i).getEigenVectors());
