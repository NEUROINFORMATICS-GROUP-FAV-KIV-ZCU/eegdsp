...
public class EigenValueDecompositionSymm {
	 ...
	 * 
	 * @param matrix
	 *            the real symmetric matrix to decomposite
	 */
			...
			if (l > i) {
				...
				do {
					...
					for (int j = l - 1; j >= i; j--) {
						...
						eigenValues[j + 1] = h + s1
								* (b1 * f + s1 * eigenValues[j]);
						for (int k = 0; k < n; k++) {
							h = eigenVectors[k][j + 1];
							eigenVectors[k][j + 1] = s1 * eigenVectors[k][j]
									+ b1 * h;
							eigenVectors[k][j] = b1 * eigenVectors[k][j] - s1
									* h;
						}
					}
